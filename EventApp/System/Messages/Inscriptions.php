array
(
   "Inscriptions_Add_Event_Title" => array
   (
      "Name" => "Selecione Evento",
      "Name_ES" => "Selecione Evento",
      "Name_UK" => "Select Event",
      "Title" => "Selecione Evento",
      "Title_ES" => "Selecione Evento",
      "Title_UK" => "Select Event",
   ),
   "Inscriptions_Receit_Title" => array
   (
      "Name" => "Recibo de Inscrição",
      "Name_ES" => "Recibo de Inscrición",
      "Name_UK" => "Inscription Receit",
   ),
   "Inscriptions_Receit_SubTitle" => array
   (
      "Name" => "Processo Seletivo",
      "Name_ES" => "Processo Seletivo",
      "Name_UK" => "Selection of Candidates",
   ),
   "Inscriptions_Receit_Status_Title" => array
   (
      "Name" => "Status da Inscrição",
      "Name_ES" => "Status del Inscrición",
      "Name_UK" => "Inscription Status",
   ),
   "Inscriptions_Receit_Datas_Title" => array
   (
      "Name" => "Dados da Inscrição",
      "Name_ES" => "Dados del Inscrición",
      "Name_UK" => "Inscription Data",
   ),
);

